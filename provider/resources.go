...
import (
	...
	"github.com/mj-sakellaropoulos/pulumi-opnsense/provider/pkg/version"
...
const (
	...
	mainMod = "index" // the opnsense module
)
...
func makeDataSource(mod string, name string) tokens.ModuleMember {
	...
	return tfbridge.MakeDataSource("opnsense", mod, "get"+name)
}

func makeResource(mod string, res string) tokens.Type {
	return tfbridge.MakeResource("opnsense", mod, convertName(res))
}
...
func Provider() tfbridge.ProviderInfo {
	...
	p := shimv1.NewProvider((opnsense.Provider()).(*schema.Provider))
	...
	prov := tfbridge.ProviderInfo{
		...
		Golang: &tfbridge.GolangInfo{
			ImportBasePath: filepath.Join(
				fmt.Sprintf("github.com/mj-sakellaropoulos/pulumi-%[1]s/sdk/", "opnsense"),
